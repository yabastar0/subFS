...
local function numToFourBytes(num)
    num = num % 4294967296
...
local byteString = numToFourBytes(num)
print(byteString)
]]
...
local function setBlock(fName, bytes, bitIndex, value)
    bitIndex = bitIndex - 1
    local vhdFile = fs.open(fName,"r+b")
    ...
    if value then
        byteTable[bytePos] = bit.bor(byteTable[bytePos] or 0, bit.blshift(1, bitPos))
    else
...
local function toBin(n)
    local result = {}
    while n > 0 do
        table.insert(result, 1, string.char(n % 256))
        n = math.floor(n / 256)
    end
    return table.concat(result,"")
end

local function setBlocks(fName, bytes, bitIndexTable, value)
    local vhdFile = fs.open(fName,"r+b")
    vhdFile.seek("set",128)
    ...
    for _,bitIndex in ipairs(bitIndexTable) do
        bitIndex = bitIndex - 1
...
function subFS.addInode(fName,Itype,Iperms,Iowner,Iguid,Ifsize,Iptype,Ipointers)
    ...
    local permBytes = pad_left(toBin(Iperms),2)

    local ownerBytes = pad_left(toBin(Iowner),4)
    local groupBytes = pad_left(toBin(Iguid),4)

    local sizeBytes = pad_left(toBin(Ifsize),8)

    local timestamp = os.epoch("utc")/1000
    timestamp = numToFourBytes(timestamp)

    local pointerstr = ""
    local c = 0
    for _,pointer in ipairs(Ipointers) do
        pointerstr = pointerstr..numToFourBytes(pointer)
        c = c + 1
    end
    if c > 10 then
    ...
    elseif ptypeByte == "i" then
        for _, pointerBlock in ipairs(Ipointers) do
            vhdFile.seek("set", pointerBlock * data[5])
            for i = 1, math.floor(data[5] / 4) do
                pointerDat = vhdFile.read(4)
                if pointerDat ~= string.rep("\0", 4) then
                    blockTrack[#blockTrack + 1] = string.unpack("<I4", pointerDat)
                end
            end
        end
    elseif ptypeByte == "2" then
        for _, dipBlock in ipairs(Ipointers) do
            vhdFile.seek("set", dipBlock * data[5])

            local sipBlocks = {}
            for i = 1, math.floor(data[5] / 4) do
                sipDat = vhdFile.read(4)
                if sipDat ~= string.rep("\0", 4) then
                    sipBlocks[#sipBlocks + 1] = string.unpack("<I4", sipDat)
                end
            end

            for _, sipBlock in ipairs(sipBlocks) do
                vhdFile.seek("set", sipBlock * data[5])
    ...
    setBlocks(fName,math.ceil(data[4]/8),blockTrack,true)

    vhdFile.seek("set",128+math.ceil(data[4]/8)+iPos)

    local wInodeDat = typeByte..permBytes..ownerBytes..groupBytes..sizeBytes..ptypeByte..pointerstr..timestamp
    vhdFile.write(wInodeDat)
    vhdFile.close()
end
